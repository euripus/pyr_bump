...
struct AlphaState
{
	...
	bool operator==(AlphaState const & other)
	{
		return (blend_enabled == other.blend_enabled) &&
				...
				(compare_enabled == other.compare_enabled) &&
				(compare == other.compare) &&  
				(reference == other.reference) &&              // TODO: epsilon compare
...
struct OffsetState
{
	...
	bool operator==(OffsetState const & other)
	{
		return (fill_enabled == other.fill_enabled) &&
				...
				(point_enabled == other.point_enabled)&&
				(scale == other.scale) &&                     // TODO: epsilon compare
				(bias == other.bias);
	}
};
...
struct StencilState
{
	...
	bool operator==(StencilState const & other)
	{
		return (enabled == other.enabled) &&
				...
				(on_fail == other.on_fail) &&  
				(on_z_fail == other.on_z_fail) &&  
				(on_z_pass == other.on_z_pass);
	}
};
...
struct WireState
{
	...
	bool operator==(WireState const & other)
	{
		return (enabled == other.enabled); 
	}
};
...
public:
	...
	// pass a null pointer.
	void setAlphaState (const AlphaState & new_state);
	void setCullState (const CullState & new_state);
	void setDepthState (const DepthState & new_state);
	void setOffsetState (const OffsetState & new_state);
	void setStencilState (const StencilState & new_state);
	void setWireState (const WireState & new_state);
};